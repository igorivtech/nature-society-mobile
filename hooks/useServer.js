...
export const useServer = () => {
  ...
  const getExplorePlaces = async (location, page) => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/getAll?lat=${location.latitude}&lng=${location.longitude}&skip=${page}&limit=10&ignoreRankings=${false}`, {
        method: "GET",
      });
  ...
  const getPlaces = (searchId, coordinate, location, radius, limit = 13, ignoreRankings = false) => {
    ...
    return new Promise((resolve) => {
      fetch(`${BASE_URL}/getAll?lat=${coordinate.latitude}&lng=${coordinate.longitude}&skip=0&limit=${limit}&radius=${1000 * radius}&ignoreRankings=${ignoreRankings}`, {
        method: "GET",

...
import { SCREEN_ASPECT_RATIO, height, DEFAULT_NOTIFICATION, INITIAL_REGION, width, SPLASH_HIDE_DELAY, MAP_BOUNDARY_NORTHEAST, MAP_BOUNDARY_SOUTHWEST, MAP_MIN_ZOOM } from "../../values/consts";
import { MAP_STYLE } from "../../values/map_style";
import { CARD_TRANSLATE_Y, ITEM_HEIGHT, ITEM_WIDTH, PlaceCard, spacerStyle, SPACER_ITEM_SIZE } from "../views/home/PlaceCard";
...
export const HomeScreen = ({ navigation, route }) => {
  ...
  // const
  // const [locationPermission, askLocation] = Location.
  ...
  const checkUserMarkerVisible = () => {
    if (location != null && mapRef.current?.__lastRegion != null) {
      ...
      if (isLocationInIsrael(location)) {
        ...
        // so that center map button never shown
    ...
    <View style={globalStyles.homeContainer}>
      ...
      >
        {sortedPlaces.map((p, index) => (
          <PlaceMarker key={index} selected={selectedPlace?.key === p.key} index={index} onPress={markerPressed} place={p} />
        ))}
        {location != null ? <UserMarker user={user} location={location} /> : null}
      </MapView>
      ...
      <AnimatedSafeAreaView style={globalStyles.cardContainerTranslateY(listYTranslate, listOpacity)}>
        <Animated.FlatList
          ...
          renderItem={({ item, index }) => {
            ...
            return <PlaceCard settings={settings} user={user} callback={showPlace} index={index} item={item} scrollX={scrollX} />;
          }}
        />
      </AnimatedSafeAreaView>
      <GrowthPoints />
      <CenterMapButton progressVisible={progressVisible} userMarkerVisible={userMarkerVisible} onPress={centerMap} />
      <Popup permissions={true} textData={strings.popups.locationPermissions} action={askLocationPermissions} popupVisible={popupVisible} setPopupVisible={setPopupVisible} />
      <Popup permissions={true} textData={strings.popups.pushPermissions} action={askPush} popupVisible={pushPopupVisible} setPopupVisible={setPushPopupVisible} />
    </View>
  );
};

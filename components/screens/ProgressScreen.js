...
import { View, Text, StyleSheet, TouchableWithoutFeedback, SafeAreaView, TouchableOpacity, Image, Animated, Easing, Alert, ScrollView } from "react-native";
            ...
            Auth.signOut()
              ...
              .finally(() => {
                ...
                AsyncStorage.clear().then(() => {
                  (async () => {
                    Alert.alert("כעט סגרו את האפליקציה", { cancelable: false });
                  })();
                });
              });
          },
    ...
    if (data.length > 0) {
      ...
      AsyncStorage.getItem(ALREADY_SHOWN).then((alreadyShown) => {
        ...
        Animated.timing(pathOpacity, {
          ...
          if (!isMounted?.current || alreadyShown) {
            return;
          }
          if (currentIndex > 0) {
        ...
        <View style={styles.progressScreenContainer}>
          ...
          >
            <Animated.View style={styles.pathContainer(translateY, pathHeight * (settings.achievements.length / 2))}>
              {data.map((item, index) => (
                <PathSegment pathHeight={pathHeight} key={index.toString()} currentIndex={currentIndex} popupVisible={popupVisible} index={index} scrollY={scrollY} item={item} />
              ))}
            </Animated.View>
          </AnimatedScrollView>
          ...
          <TouchableOpacity style={styles.bottomButtonContainer} onPress={loginLogout}>
            {user ? (
              ...
              <Animatable.Text useNativeDriver={true} easing="ease-out" animation="pulse" iterationCount="infinite" iterationDelay={200} style={styles.bottomText}>
...
const ProgressPopup = () => {
  ...
  return (
    <Animated.View style={pStyles.container(scale)}>
      {notification != null ? <NotificationHeader notification={notification} /> : null}

      {notification != null ? <Text style={textStyles.normalOfSize(16)}>{notification.description}</Text> : null}
